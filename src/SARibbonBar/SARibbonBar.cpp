...
public:
	...
	SARibbonBarPrivate(SARibbonBar *par)
		: applicationButton(nullptr), ribbonTabBar(nullptr), stackedContainerWidget(nullptr), iconRightBorderPosition(1), minimumCaterogyButton(nullptr), rightButtonGroup(nullptr), ribbonStyle(SARibbonBar::OfficeStyle), lastShowStyle(SARibbonBar::OfficeStyle), currentRibbonMode(SARibbonBar::NormalRibbonMode), windowButtonSize(100, RibbonSubElementStyleOpt.titleBarHight), mContextCategoryColorListIndex(-1)
	{
		MainClass = par;
...
void SARibbonBar::showMinimumModeButton(bool isShow)
{
	if (isShow)
	{
		...
		if (nullptr == m_d->minimumCaterogyButton)
		{
			...
			action->setIcon(style()->standardIcon(isMinimumMode() ? QStyle::SP_TitleBarUnshadeButton : QStyle::SP_TitleBarShadeButton, 0, m_d->minimumCaterogyButton));
		...
		if (isMinimumMode())
		{
			...
			if (!m_d->stackedContainerWidget->isVisible())
			{
				if (m_d->stackedContainerWidget->isPopupMode())
				{
					...
					QHoverEvent ehl(QEvent::HoverLeave, m_d->ribbonTabBar->mapToGlobal(QCursor::pos()), m_d->ribbonTabBar->mapToGlobal(QCursor::pos()));
...
void SARibbonBar::paintInNormalStyle()
{
	...
	if (parWindow)
	{
		...
		{
			titleRegion.setRect(m_d->quickAccessBar->geometry().right() + 1, RibbonSubElementStyleOpt.widgetBord.top(), width() - m_d->iconRightBorderPosition - RibbonSubElementStyleOpt.widgetBord.right() - m_d->windowButtonSize.width() - m_d->quickAccessBar->geometry().right() - 1, RibbonSubElementStyleOpt.titleBarHight);
		}
		else
		{
			...
			if (rightwidth > leftwidth)
			{
				...
				titleRegion.setRect(contextCategoryRegion.y(), RibbonSubElementStyleOpt.widgetBord.top(), rightwidth, RibbonSubElementStyleOpt.titleBarHight);
			}
			else
			{
				//说明左边的大一点
				titleRegion.setRect(m_d->iconRightBorderPosition + m_d->quickAccessBar->geometry().right(), RibbonSubElementStyleOpt.widgetBord.top(), leftwidth, RibbonSubElementStyleOpt.titleBarHight);
			}
		}
#ifdef SA_RIBBON_DEBUG_HELP_DRAW
		p.save();
...
void SARibbonBar::paintInWpsLiteStyle()
{
	...
	if (parWindow)
	{
		...
		if (width > 20)
		{
			QRect titleRegion(start, RibbonSubElementStyleOpt.widgetBord.top(), width, RibbonSubElementStyleOpt.titleBarHight);
#ifdef SA_RIBBON_DEBUG_HELP_DRAW
			p.save();
			p.setBrush(QColor(255, 0, 0, 120));
			p.drawRect(titleRegion);
...
void SARibbonBar::resizeStackedContainerWidget()
{
	if (m_d->stackedContainerWidget->isPopupMode())
	{
		...
		m_d->stackedContainerWidget->setGeometry(absPosition.x(), absPosition.y(), width() - RibbonSubElementStyleOpt.widgetBord.left() - RibbonSubElementStyleOpt.widgetBord.right(), mainBarHeight() - m_d->ribbonTabBar->geometry().bottom() - RibbonSubElementStyleOpt.widgetBord.bottom() - 1);
	}
	else
	{
		m_d->stackedContainerWidget->setGeometry(RibbonSubElementStyleOpt.widgetBord.left(), m_d->ribbonTabBar->geometry().bottom() + 1, width() - RibbonSubElementStyleOpt.widgetBord.left() - RibbonSubElementStyleOpt.widgetBord.right(), mainBarHeight() - m_d->ribbonTabBar->geometry().bottom() - RibbonSubElementStyleOpt.widgetBord.bottom() - 1);
	}
}
